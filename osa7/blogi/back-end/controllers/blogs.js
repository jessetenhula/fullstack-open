...
blogsRouter.post('/', userExtractor, (request, response, next) => {
  ...
  if(!request.body.hasOwnProperty('comments')) {
    blog.comments = []
  }
...
blogsRouter.delete('/:id', userExtractor, async (request, response, next) => {
  try {
    ...
    await user.save()
    for(let count = 0; count < blog.comments.length; count++) {
      await Comment.findByIdAndDelete(blog.comments[count])
    }
...
blogsRouter.put('/:id', async (request, response, next) => {
  ...
  try {
    ...
    await newBlog.populate('comments', { content: 1, id: 1 })
